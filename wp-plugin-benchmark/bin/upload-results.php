...
$telemetry = array(
    ...
    'plugin_speed_percentage' => isset($data['percentage_ms']) ? $data['percentage_ms'] : (isset($data['baseline_time_ms']) && $data['baseline_time_ms'] > 0 ? round(($data['delta_ms'] / $data['baseline_time_ms']) * 100, 1) : 0),
    'baseline_site_load_speed' => $data['baseline_time_ms'],
    'plugin_error' => $data['error'],
    'error_category' => $data['error_category'] ?? 'none',
    'scanner_engine_version' => $data['scanner_engine_version'] ?? getenv('SCANNER_ENGINE_VERSION') ?: '0.1.0',
    'env' => (object)$env,
